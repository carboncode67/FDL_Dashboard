...
- [ ] 50. **Drone Flights Table** Drones are a component in the table, but experiment level, individual drone flights will need to be added and given several data storage options and fields. The goal is to be able to schedule and track drone flights from Flight> raw data ingest> ortho/3d processing> image annotation for ai pipelines> feature extraction. Drone imagery/products will not be stored in the db, but linked to it and stored on the zraid 1 array in the lab server. Polygon features such as feature tags created to train feature extraction pipelines, or automatically segmented features, will be stored in the DB. When a drone flight is scheduled in an experiment, a new db entry is added that includes the drone assigned, the assignment date and the following empty fields:
	- [ ] Total acres
	- [ ] Total images
...
- [ ] 53. **Show tasks in experiment page and dashboard** When on 
- [ ] 54. **Link treatment to specific fields in experiment page**
- [ ] 56. **Address Geocoder** Use usgs or other api to convert farm addresses to lat lon.
...
- [ ] 58. **Link uploaded data to fields by gps coordinates** In the db table for each data upload, there should be a link to a field if the data's gps metadata intersects it. Recordings and tracks can be assigned multiple fields if they cross field boundaries. 
