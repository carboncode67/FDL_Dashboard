...
export function BulkSendOnboardingEmailModal({
  ...
  const [statuses, setStatuses] = useState<Record<number, { status: SendStatus; error?: string }>>({});
  ...
  async function handleSend() {
    ...
    for (const r of recipients) {
      ...
      try {
        const res = await fetch(`/api/contacts/${r.id}/send-onboarding-email`, {
          method: "POST",
  ...
  return (
    <Dialog open={open} onOpenChange={(next) => { if (!next && !sending) onClose(); }}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>
            Send onboarding email to {recipients.length} contact{recipients.length !== 1 ? "s" : ""}
          </DialogTitle>
          <DialogDescription>
