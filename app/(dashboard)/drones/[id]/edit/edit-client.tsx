    ...
    <div className="max-w-2xl space-y-6">
      <div>
        <div className="flex items-center gap-2 text-sm text-stone-500 mb-1">
          <Link href="/drones" className="hover:text-stone-900">Equipment</Link>
          <span>/</span>
          <span>Edit</span>
        </div>
        <h2 className="text-2xl font-bold text-stone-900">Edit {drone.Name ?? `Equipment #${drone.id}`}</h2>
      </div>
      ...
      <div className="bg-white border rounded-lg p-6 space-y-3">
        <div className="flex items-center justify-between pb-2 border-b">
          <h3 className="text-sm font-semibold text-stone-900">Data Tables</h3>
          <Link
          ...
          <ul className="divide-y">
            {dataTables.map((t) => (
              <li key={t.id} className="flex items-center justify-between py-2 text-sm">
                ...
                <span className="text-xs text-stone-400">{t.columnCount} column{t.columnCount === 1 ? "" : "s"}</span>
              </li>
            ))}
          </ul>
        )}
