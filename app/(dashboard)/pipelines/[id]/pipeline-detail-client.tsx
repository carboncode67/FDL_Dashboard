  ...
  return (
    <div className="space-y-4">
      <div className="flex items-start justify-between">
        <div>
          <h2 className="text-2xl font-bold text-stone-900">{pipeline.name}</h2>
          {pipeline.description && <p className="text-sm text-stone-500 mt-1">{pipeline.description}</p>}
        ...
        <Card>
          <CardContent className="pt-4 space-y-3">
            ...
            <form ref={editFormRef} onSubmit={handleEditSubmit} className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              <div className="space-y-1">
                <label className="text-sm font-medium text-stone-700">Replace sample dataset (optional)</label>
                <p className="text-xs text-stone-500">Currently: {pipeline.sample_dataset_original_name}</p>
                <input type="file" name="sample_dataset" className="text-sm" />
              </div>
              <div className="space-y-1">
                <label className="text-sm font-medium text-stone-700">Replace script (optional)</label>
                <p className="text-xs text-stone-500">Currently: {pipeline.script_original_name}</p>
                <input type="file" name="script" className="text-sm" />
              </div>
              <div className="space-y-1 sm:col-span-2">
                <label className="text-sm font-medium text-stone-700">Replace model weights (optional)</label>
      ...
      <Card>
        <CardContent className="pt-4 space-y-2 text-sm">
          <div><span className="font-medium text-stone-700">Sample dataset:</span>{" "}
            <a className="text-green-700 hover:underline" href={`/api/files/pipeline-datasets/${pipeline.sample_dataset_filename}`} target="_blank" rel="noreferrer">
              {pipeline.sample_dataset_original_name}
            </a>
          </div>
          <div><span className="font-medium text-stone-700">Script:</span>{" "}
            <a className="text-green-700 hover:underline" href={`/api/files/pipeline-scripts/${pipeline.script_filename}`} target="_blank" rel="noreferrer">
              {pipeline.script_original_name}
            </a>
          </div>
          {pipeline.model_filename && (
            <div><span className="font-medium text-stone-700">Model:</span>{" "}
              <a className="text-green-700 hover:underline" href={`/api/files/pipeline-models/${pipeline.model_filename}`} target="_blank" rel="noreferrer">
          ...
          {pipeline.wired_command && (
            <div>
              <span className="font-medium text-stone-700">Wired command:</span>{" "}
              <code className="rounded bg-stone-100 px-1.5 py-0.5 text-xs">{pipeline.wired_command}</code>
            </div>
          )}
          {pipeline.llm_notes && (
            <div>
              <span className="font-medium text-stone-700">LLM notes:</span>
              <p className="mt-1 whitespace-pre-wrap text-stone-600">{pipeline.llm_notes}</p>
            </div>
          )}
          <div className="text-stone-500">Created by {pipeline.creator_name} on {new Date(pipeline.created_at).toLocaleDateString()}</div>
        </CardContent>
      </Card>

      <h3 className="text-lg font-semibold text-stone-900">Run history</h3>
        ...
        <Table>
          ...
          <TableBody>
            {pipeline.runs.length === 0 ? (
              <TableRow>
                <TableCell colSpan={6} className="text-center text-stone-500 py-8">No runs yet</TableCell>
              </TableRow>
            ) : pipeline.runs.map((r) => (
              <Fragment key={r.id}>
                <TableRow className="cursor-pointer hover:bg-stone-50"
                  ...
                  <TableCell className="text-sm text-stone-600">{r.is_test_run ? "test" : "triggered"}</TableCell>
                  <TableCell className="text-sm text-stone-500">{r.started_at ? new Date(r.started_at).toLocaleString() : "—"}</TableCell>
                  <TableCell className="text-sm text-stone-500">{r.finished_at ? new Date(r.finished_at).toLocaleString() : "—"}</TableCell>
                  <TableCell className="text-sm text-stone-500">{r.output_files.length || "—"}</TableCell>
                </TableRow>
                {expandedRun === r.id && (
                  <TableRow key={`${r.id}-detail`}>
                    <TableCell colSpan={6} className="bg-stone-50 text-xs space-y-2">
                      ...
                      {r.output_files.length > 0 && (
                        <div>
                          <p className="font-medium text-stone-700">Outputs</p>
                          <ul className="list-disc list-inside">
                            {r.output_files.map((f) => (
                              <li key={f.filename}>
                                <a className="text-green-700 hover:underline" href={f.download_url} target="_blank" rel="noreferrer">{f.filename}</a>
                              </li>
                            ))}
                          </ul>
                        </div>
