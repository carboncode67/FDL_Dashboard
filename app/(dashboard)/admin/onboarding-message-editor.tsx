...
export function OnboardingMessageEditor({ initialMessage }: OnboardingMessageEditorProps) {
  ...
  return (
    <div className="space-y-3">
      <textarea
        className="w-full border border-stone-200 rounded-lg p-3 text-sm text-stone-800 resize-y focus:outline-none focus:ring-2 focus:ring-green-500"
        rows={10}
        value={message}
      ...
      <div className="flex items-center gap-2">
        <button
          ...
          className="inline-flex items-center justify-center rounded-lg bg-green-600 px-3 py-1.5 text-sm font-medium text-white hover:bg-green-700 disabled:opacity-50"
        >
          {saving ? "Saving..." : "Save"}
        </button>
        {dirty && (
