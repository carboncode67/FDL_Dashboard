...
**Login rate limiting** (`lib/rate-limit.ts`): In-memory sliding window, 10 attempts per IP per 15 minutes. Reads `CF-Connecting-IP` header first (Cloudflare tunnel), then `x-real-ip`, then `x-forwarded-for`. If fronted by Nginx Proxy Manager behind Cloudflare, that first header needs `proxy_set_header CF-Connecting-IP $http_cf_connecting_ip;` — but confirmed as of 2026-09-15 that **TrueNAS dev (`fdl.casata.org`) actually runs Caddy + Unbound on an OPNsense router**, not NPM/Cloudflare; Caddy sets `X-Forwarded-For` by default so the fallback tier still works there, it just never hits the first (Cloudflare-specific) header. What fronts **production** hasn't been re-verified against this — root `CLAUDE.md`'s network diagram for production only mentions the Pangolin tunnel, not NPM, so treat that part as unconfirmed too rather than assuming it still matches.
...
- **CRS verification:** every call that can trigger a run (`app/api/pipelines/[id]/run/route.ts`, `runs/[runId]/rerun/route.ts`, `lib/pipeline-match.ts`) resolves the run's farm (as it always did, for `PipelineRun.farm_id`) and now also fetches that farm's geocoded `latitude`/`longitude` via `lib/pipeline-farm.ts`'s `farmCentroidFor()` and sends it as `TriggerRunPayload.farm_centroid` — ground truth the processor's `geo_sanity.py` checks output rasters/vectors against before ever exposing them (see `PipelineProcessor/CLAUDE.md`). `output_files` entries can now carry `kind: "vector"` (in addition to `"raster"`/`"file"`) and `crs_status: "ok" | "unclear"` + `crs_epsg`; `ingestOutputRasters()` in the webhook route persists these onto `Pipeline_Output_Rasters` (`kind`, `crs_status`, `crs_epsg` — migration `059_pipeline_output_crs.sql`). `components/farm-map.tsx` never plots a row with `crs_status: "unclear"` as a map layer (listed separately below the map as needing manual review instead) and renders `kind: "vector"` rows as a GeoJSON layer rather than a `GeoRasterLayer`.
...
**Basemaps — large raster upload & tiling** (`app/api/basemaps/`, `components/basemap-upload.tsx`): items 6+7 — see `docs/raster-tiling-plan.md`. A lab member uploads a large raster (typically a drone orthomosaic GeoTIFF, 1-5 GB) via `BasemapUpload` on a farm's Overview tab; `POST /api/basemaps` streams it to `DATA_DIR/basemap-sources/` with `busboy` (same never-buffer-a-huge-file-in-the-heap reasoning as the recording upload route), creates a `Basemaps` row (`tiling_status: "pending"`), and — reusing `processingConfigured`/`PROCESSING_URL`/`PROCESSING_API_KEY` from the pipelines feature above, no new env vars — fires `triggerBasemapTiling()` (`lib/processing.ts`) at PipelineProcessor's `POST /basemaps/{id}/tile`, passing a `farm_centroid` (`lib/pipeline-farm.ts`'s `farmCentroidFor()`) for the same CRS-verification purpose `TriggerRunPayload.farm_centroid` serves for pipeline runs.

- **Tiling is static/pre-generated, not a live tile server** — PipelineProcessor's `basemap_tiling.py` (its own `CLAUDE.md`) COG-ifies the upload, reuses `geo_sanity.py` to normalize its CRS, tiles it with `rio-tiler`, zips the whole `{z}/{x}/{y}.png` pyramid into one archive, and calls back `POST /api/basemaps/webhook` (HMAC-verified via `lib/webhook-auth.ts`, shared with the pipelines webhook — same `PROCESSING_WEBHOOK_SECRET`) with `footprint`/`crs_status`/`crs_epsg`/zoom range + a `tile_archive_url`. The webhook streams that one zip down (`yauzl`, same lazy-entry pattern as `lib/context-fetch.ts`'s GeoDaRT zip handling) into `DATA_DIR/basemap-tiles/<basemap_id>/`, then flips `tiling_status` to `"ready"`.
- **Serving tiles**: `GET /api/basemaps/[id]/tiles/[z]/[x]/[y]/route.ts` is a dedicated route, not an extension of `GET /api/files/[type]/[filename]` — that route only ever serves one flat `path.basename` segment, no `{z}/{x}/{y}` nesting. Session-authed like any other dashboard asset (the browser's session cookie rides along on a same-origin tile request automatically). The raw uploaded source file itself, by contrast, *is* served through the ordinary `/api/files/basemap-sources/[filename]` route (added to its `ALLOWED_TYPES`) — that route's non-range fallback was changed from `fs.readFileSync` to a streamed response specifically because of this feature (a multi-GB basemap source, unlike every other `ALLOWED_TYPES` entry, shouldn't be buffered whole into the Node heap).
- **Display**: `components/map-raster-layers.tsx`'s `MapRaster` gained a `source: "pipeline" | "basemap"` discriminant (Basemap.id and PipelineOutputRaster.id are separate, colliding ID spaces — the farm-map/sampling-map-editor checklists' checkbox `Set` is keyed by `` `${source}-${id}` ``, not the bare id) and a `TiledBasemapLayer` component (plain `L.tileLayer` against the route above, bounded by the stored `footprint`) alongside the existing whole-file `RasterLayer`/`VectorLayer`. `lib/map-rasters.ts`'s `pipelineOutputToMapRaster()`/`basemapToMapRaster()` are the shared mapping functions both `farms/[id]/page.tsx` and `farms/[id]/maps/[mapId]/page.tsx` call to merge the two source tables into one `rasters` list — a `Basemaps` row is only ever included once `tiling_status: "ready"`.
- **Sampling-map basemap toolset** (item 6.2, `components/sampling-map-editor.tsx` header toolbar, next to the existing Linked Form/Auto-prompt radius controls): `Sampling_Maps.basemap_id`/`basemap_buffer_m` (positive — extends *beyond* the field boundary, the opposite sign from the point-generation toolset's inward edge buffer)/`basemap_max_zoom`, same one-setting-per-map/save-from-the-toolbar precedent as `form_id`/`proximity_radius_m`. Phase 1 (this feature) only persists the setting and lets the chosen basemap preview in the editor via the same checklist mechanism; actually clipping+packaging an MBTiles subset for the phone apps (and displaying it there — Swift has no native SwiftUI raster-tile-overlay API, unlike Kotlin's Google Maps Compose `TileProvider`) is Phase 2, not yet built — see `docs/raster-tiling-plan.md`.
